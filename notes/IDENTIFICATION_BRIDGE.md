...
In-paper anchor: `paper/LOCAL_GLOBAL_LANGLANDS_COMPATIBILITY_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of admissible compatibility correspondences and local-global matching packages through a multi-lane Langlands compatibility super-architecture`.
...
- endpoint lock: `reciprocity_transfer`.
...
2. the endpoint-transfer lock `reciprocity_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_LGC` remains positive.
...
Payload: verify `reciprocity_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_LGC` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `reciprocity_transfer`
